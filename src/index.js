...
function removeSelective(path, t, directiveTriggers, commentTriggers) {
  if (path.node.value.value === "use strict") {
    ...
    for (const sibling of siblings) {
      if (t.isDirective(sibling)) {
        // if the sibiling is a directive
        if (
          sibling.value.value === "not strict" ||
          (commentTriggers &&
            directiveTriggers.includes &&
            directiveTriggers.includes(sibling.value.value)) // extra directive triggers
        ) {
      ...
      Directive(path, state) {
        if (state.opts.removeAll) {
          ...
          removeSelective(
            path,
            t,
            state.opts.directiveTriggers,
            state.opts.commentTriggers
          ); // remove for file that only have 'not strict'
        }
      },
    },
